...
#define UART_RX_BUFFER_SIZE (YMODEM_PACKET_1024 + 16)
typedef struct
{
    uint8_t uart_rx_buffer[UART_RX_BUFFER_SIZE];
    uint8_t uart_rx_frame_ready;   // 标志位，表示有完整帧数据准备好
    uint16_t uart_rx_head;         // 接收缓冲区头指针
    uint16_t uart_rx_tail;         // 接收缓冲区头尾指针
    uint16_t uart_rx_frame_length; // 当前帧长度
} usart_buffer_s;

/**
 * @brief 串口初始化
 * @note 用户需要实现此函数，完成UART硬件初始化，包括：
 *       - GPIO配置（TX/RX引脚）
 *       - UART参数配置（波特率、数据位、停止位、校验位）
 *       - 时钟使能
 *       - 中断配置（如果使用中断方式）
 */
...
/**
 * @brief 串口去初始化
 * @note 在跳转到应用程序前调用，完成UART硬件去初始化，包括：
 *       - 禁用UART中断
 *       - 关闭UART外设
 *       - 复位GPIO引脚
 */
 ...
 */
uint16_t IAP_UART_ReceiveData(uint8_t *buffer, uint16_t max_length, uint32_t timeout);
